...
(function () {
  ...
  carousels.forEach(function (carousel) {
    ...
    function goToSlide(index) {
      ...
      // .room-carousel-dots lives inside .room-carousel-track (matches the Figma
      // structure), so it slides along with the track by default. Counter-shift
      // it by the same amount in the opposite direction so it stays visually
      // fixed in place regardless of which slide is active.
      if (dotsWrap) {
...
(function () {
  ...
  function parseCurrency(text) {
    // Match only a proper "digits[,digits]*.digits" number, so a
    // "Rs." prefix's own period doesn't get pulled into the parse.
